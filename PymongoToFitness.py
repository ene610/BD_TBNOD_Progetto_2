...
rdd = rdd.map(RowUserFollowerEngagement).reduceByKey(countStatusTweet).map(ordinaTupla).filter(lambda x: x[2]>3)
...
def index():
    ...
    return render_template("PymongoFitness.html", data1=string_Json)
