...
df01.show()
rdd = df01.rdd.map(lambda x : x[1:]).filter(lambda x : int(x[7])>500).map(lambda x : (x[15],x[14],x[7],x[11],x[12],x[13]))
rdd = rdd.map(RowUserFollowerEngagement).reduceByKey(countStatusTweet).map(ordinaTupla).filter(lambda x: x[2]>5)
...
def index():
    ...
    return render_template("PymongoMusica.html", data1=string_Json)
