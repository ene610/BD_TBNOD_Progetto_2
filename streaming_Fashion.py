...
url = 'http://localhost:5003/Fashion/'
...
sc = SparkContext(appName = "fashion")
ssc = StreamingContext(sc,10) # (sparkContext, batchDuration

kafkaStream = KafkaUtils.createDirectStream(ssc, ['Fashion'], {'metadata.broker.list': 'localhost:9092'})
...
def JobWordCount(rdd):
    ...
    lines = rdd.mapValues(lambda x: json.loads(x)).mapValues(filterWord).flatMap(lambda x: x[1].split(" ")) \
        .map(lambda x: re.sub(stopword_regex, "", " " + x + " ")).map(lambda x: re.sub(" ", "", x))\
        .filter(lambda x: x != "")
