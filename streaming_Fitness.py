...
kafkaStream = KafkaUtils.createDirectStream(ssc, ['Fitness'], {'metadata.broker.list': 'localhost:9092'})
...
def JobWordCount(rdd):
    ...
    lines = rdd.mapValues(lambda x: json.loads(x)).mapValues(filterWord).flatMap(lambda x: x[1].split(" ")) \
        .map(lambda x: re.sub(stopword_regex, "", " " + x + " ")).map(lambda x: re.sub(" ", "", x)).filter(
        lambda x: x != "")
